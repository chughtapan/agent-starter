...
import { AmbiguousCompletionError, MailboxError } from './errors.js';
...
const ATTENTION_LABELS: Partial<Record<UpdateKind, string>> = {
  needsYou: MESSAGE_LABELS.needsYou,
  ready: MESSAGE_LABELS.ready,
};
const PROGRESS_LABELS: Partial<Record<UpdateKind, string>> = {
  waiting: THREAD_LABELS.waiting,
  working: THREAD_LABELS.working,
  failed: THREAD_LABELS.failed,
};
...
const MailMessageWire = Schema.Struct({
  ...
  to: Schema.optionalKey(Schema.Array(SenderWire)),
...
export function classifyUpdateKind(
  ...
  if (messageLabels.has(THREAD_LABELS.failed)) {
    return 'failed';
  }
  if (messageLabels.has(THREAD_LABELS.working)) {
    return 'working';
  }
  if (messageLabels.has(THREAD_LABELS.waiting)) {
    return 'waiting';
  }
...
function classifyThread(
  ...
  return CollaborationUpdate.make({
    ...
    collaborator: inbound
      ? senderText(message.from ?? message.sender)
      : senderText(
          message.to?.find(
            (recipient) => senderEmail(recipient) !== inbox.toLowerCase(),
          ),
        ),
...
function summarize(
  ...
  const source = message.subject ?? thread.subject ?? 'collaboration update';
  const firstLine =
    source.replace(COLLABORATION_SUBJECT, '').split('\n')[0]?.trim() ?? '';
...
function mailboxFailure(operation: string, reason: string): MailboxError {
...
function decodeResponse<S extends Schema.Top>(
  ...
  return Schema.decodeUnknownEffect(schema)(value).pipe(
    Effect.mapError(() =>
      mailboxFailure(operation, 'AgentMail response failed schema validation'),
    ),
  );
}
...
function sendResult(value: unknown): Effect.Effect<SendResult, MailboxError> {
  return Schema.decodeUnknownEffect(
    ...
    Effect.mapError(() =>
      mailboxFailure('send', 'AgentMail response failed schema validation'),
    ),
  );
}
  ...
  Effect.gen(function* () {
      ...
      const response = yield* httpClient
        ...
        .pipe(
          Effect.mapError(() =>
            mailboxFailure(
              operation,
              'AgentMail request failed; check connectivity and credentials',
            ),
          ),
        );
      return yield* response.json.pipe(
        Effect.mapError(() =>
          mailboxFailure(operation, 'response was not JSON'),
        ),
      );
    });
    ...
    const executePublicJson = Effect.fn('Mailbox.executePublicJson')(function* (
      ...
      const withBody = yield* request.pipe(
        ...
        Effect.mapError(() =>
          mailboxFailure(operation, 'request body was not JSON'),
        ),
      );
      const response = yield* httpClient
        .execute(withBody)
        .pipe(
          Effect.mapError(() =>
            mailboxFailure(
              operation,
              'AgentMail request failed; check connectivity and signup details',
            ),
          ),
        );
      return yield* response.json.pipe(
    ...
    const listAllThreads = (operation: string) => {
      ...
      > =>
        Effect.gen(function* () {
          ...
          if (
            visited.includes(list.next_page_token) ||
            list.next_page_token === pageToken ||
            visited.length >= 1000
          ) {
            return yield* Effect.fail(
              mailboxFailure(
                operation,
                'AgentMail pagination repeated a token or exceeded its safety limit',
              ),
            );
          }
          return [
    ...
    const triage = Effect.fn('Mailbox.triage')(function* (
      messageId: string,
      kind: UpdateKind,
    ) {
      const update = (yield* listUpdates()).find(
        (item) => item.messageId === messageId,
      );
      if (update === undefined) {
        return yield* Effect.fail(
          mailboxFailure('triage', 'the selected message is no longer open'),
        );
      }
      const attention = ATTENTION_LABELS[kind];
      const progress = PROGRESS_LABELS[kind];
      yield* updateThread(
        update.threadId,
        [
          THREAD_LABELS.collaboration,
        ...
        ].filter((label) => label !== progress),
      );
      yield* updateMessage(
        messageId,
        [
          MESSAGE_LABELS.triaged,
        ...
        [
          ...
          THREAD_LABELS.working,
          THREAD_LABELS.failed,
        ].filter((label) => label !== attention && label !== progress),
      );
    });
