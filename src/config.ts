...
import { HarnessConfig, makeDefaultConfig } from './domain/configuration.js';
import { ConfigurationError, type StorageError } from './errors.js';
  ...
  Effect.gen(function* () {
    ...
    const save = Effect.fn('Configuration.save')(function* (
      ...
      const encoded = yield* Schema.encodeEffect(HarnessConfig)(config).pipe(
        Effect.mapError((error) =>
          ConfigurationError.make({ field: 'config', reason: error.message }),
        ),
      );
