...
export const pathsLayer = Layer.effect(Paths)(
  Effect.gen(function* () {
    ...
    const systemHome = yield* Config.string('HOME');
    const userHome = yield* Config.string('SOCIAL_HARNESS_USER_HOME').pipe(
      Config.withDefault(systemHome),
    );
    const claudeHome = yield* Config.string('CLAUDE_CONFIG_DIR').pipe(
      Config.withDefault(path.join(userHome, '.claude')),
    );
    const codexHome = yield* Config.string('CODEX_HOME').pipe(
      Config.withDefault(path.join(userHome, '.codex')),
    );
