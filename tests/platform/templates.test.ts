...
} from '../../src/platform/documents/index.js';
  ...
  layer(templateTestLayer)((it) => {
    it.effect('renders Alice’s complete installed identity', () =>
      Effect.gen(function* () {
        ...
        assert.include(identity, '`paths.norms`');
        assert.include(identity, '`context --json`');
      }),
    );
    ...
    it.effect(
      ...
      () =>
        Effect.gen(function* () {
          ...
          assert.include(skill, 'run `social-harness items` immediately after');
        }),
    );
